...
const API_BASE_URL = "https://dev-moveservices.mroads.co";
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setTotalSeats(draft.totalSeats ?? 4);
        setAvailableSeats(draft.availableSeats ?? 3);
        setPricePerSeat(draft.pricePerSeat ?? 150);
      ...
      <ConfirmPublishModal
        ...
        onPublish={async () => {
          ...
          try {
            ...
            // Note: distanceKm & etaMin removed from payload (Option A).
            const payload: any = {
...
function RouteStep({
  ...
  const pickSuggestion = async (item: any, type: "FROM" | "TO") => {
    ...
    if (type === "FROM") {
      ...
      if (Number.isFinite(lat) && Number.isFinite(lon)) {
        ...
        // rate-limit guard + avoid duplicate requests
        if (fromCoords) {
          const key = `${fromCoords.latitude},${fromCoords.longitude}-${coords.latitude},${coords.longitude}`;
          if (lastRouteRef.current !== key) {
            lastRouteRef.current = key;
            const info = await getRouteInfo(fromCoords, coords);
            if (info) {
              setDistanceKm(info.distanceKm);
              setEtaMin(info.durationMin);
              setRouteCoords(info.geometry);
              // recenter map to fit route by setting mapCoords to midpoint (simple)
  ...
  // Phase-1 validation: require origin selected. (Optional: require destination if desired)
  const isValid = !!fromCoords;
